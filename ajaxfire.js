var fire = Fire('series');
// var fire = Fire('parallel');
fire.set(function(each, results, index){
    setTimeout(function(){console.log("----- 1 ----- " + Date.now());each();}, 2000);
});
fire.set(function(each, results, index){
    setTimeout(function(){console.log("----- 2 ----- " + Date.now());each();}, 3000);
});
fire.set(function(each, results, index){
    setTimeout(function(){console.log("----- 3 ----- " + Date.now());each();}, 1000);
});
fire.onEach(function(results, index){
    console.log('on each', results, index);
});
fire.onDone(function(results){
    console.log('on done', results);
});
fire.start();
